<template>
  <view>
    <cmd-nav-bar back title="密码重置"></cmd-nav-bar>
    <cmd-page-body type="top">
      <cmd-transition name="fade-up">
        <view class="forgot">
          ...
          <button
            class="btn-forgot"
            :class="forgotMobile ? 'btn-forgot-active' : ''"
            :disabled="!forgotMobile"
            hover-class="btn-forgot-hover"
            @tap="fnForgot"
          >
            提交
          </button>
    ...
    mobile: {
      handler(newValue) {
        if (
          ...
          newValue.code.length == 6 &&
          newValue.passwordOne.length >= 8 &&
